...
class ForkedMain {
    ...
    private static final AtomicReference linkRef = new AtomicReference<>();
    ...
    static void hangup() {
        ...
        if (!gracefullyFinished) {
            ...
            BinaryLinkClient link = (BinaryLinkClient) linkRef.get();
            if (link != null) {
                try {
        ...
        BinaryLinkClient link = (BinaryLinkClient) linkRef.getAndSet(null);
        if (link != null) {
            try {
                link.close();
            } catch (IOException e) {
    ...
    private static class ShutdownTimeoutThread extends Thread {
        ...
        private String getMessage(long waitMore) {
            ...
            for (Map.Entry e : Thread.getAllStackTraces().entrySet()) {
                ...
                StackTraceElement[] els = (StackTraceElement[]) e.getValue();
